...
public class UpdateProfile {
	...
	public UpdateProfile(KafkaProducerService kafkaProducerService, UserService userService) {
		this.userService = userService;
		this.kafkaProducerService = kafkaProducerService;
	}
	
	@PatchMapping(path="/updateprofile/{id}")
	public ResponseEntity<Map<String, ?>>  profileUpdate(@RequestBody ProfileDto profileDto, @PathVariable Long id) {
		...
		if (idno) {
			...
			try {
				kafkaProducerService.sendEvent(KafkaEventType.UPDATE_PROFILE, map);
			} catch (Exception e) {
