...
import com.java25.java25.springboot4.mysql.dto.MfaToken;
...
public class VerifyMfa {
	...
	public VerifyMfa(KafkaProducerService kafkaProducerService, UserService userService) {
    ...
    public ResponseEntity<Map<String, String>> mfaVerifyTotp(@RequestBody MfaToken mfaToken, @PathVariable Long id) {    	
		...
		if (user != null) {
	        ...
	        if (isCodeValid) {
				...
				try {
					kafkaProducerService.sendEvent(KafkaEventType.VERIFY_OTP, map);
				} catch (Exception e) {
